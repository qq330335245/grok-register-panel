...
- 用户名可含 grok2api 同款 `{account}` / `{email}` / `{id}` 模板（例如 `socks5h://g2a.{account}:pass@host:10800`）。注册时按邮箱展开，字母数字和 `+` `-` `_` 保留，`@` `.` 等其余字符变成 `_`，不会把 `@` 编成 `%40`；探测使用占位账号
- 导入时规范化并去重，随后后台并发探活；GET 接口轮询检测进度
- API 和 UI 只返回脱敏端点及 `has_auth`，不会返回代理账号或密码
- worker 只选择“健康 + 启用”的条目，并在每个新账号创建浏览器前热加载一次
- 一个账号从注册、SSO 到 OAuth 始终固定同一代理，不会中途更换出口；动态粘性模板可被多个 worker 同时使用，靠展开后的用户名隔离出口
